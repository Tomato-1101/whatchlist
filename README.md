...
| `1` | 値上がり率だけ取得 |
| `12` | 値上がり率と値下がり率を取得 |
| `158` | 値上がり率、活況銘柄、ティック回数を取得 |
| `q` | 終了 |

💡 **ポイント**: 数字を入力すると即座に取得が開始されます！確認なしでスムーズに実行されるので、入力後は完了まで待つだけでOKです。

### 💻 コマンドラインで実行

コマンドプロンプトやターミナルから直接実行することもできます：
...
# 値上がり率ランキングを取得
python -m src.main --ranking up

# 複数のランキングを取得
python -m src.main -r up -r down -r volume

# 全ランキングを取得
python -m src.main --all

# 取得件数を指定（デフォルト50件）
python -m src.main --ranking up --count 30
```
...
#### 方法2: ファイルからインポート
1. 🌐 TradingViewを開く
2. ⭐ ウォッチリストの「...」メニューをクリック
3. 📥 「リストをインポート」を選択
4. 📂 `output`フォルダ内のテキストファイルを選択
5. ✅ 完了！
